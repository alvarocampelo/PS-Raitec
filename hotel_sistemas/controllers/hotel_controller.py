...
class HotelController:
    ...
    def reqBooking(self, booking: Booking):
        ...
        insert_query = """
            INSERT INTO bookings (client_cpf, room_number, checkin, checkout, daily_price)
            VALUES (%s, %s, %s, %s, %s)
        """
...
# --- INSIGHTS E LISTAGENS ---
            ...
            with conn.cursor(dictionary=True) as cursor:
                query = """
                    SELECT b.id, c.name as client_name, b.room_number, 
                        b.checkin, b.checkout, b.active, b.daily_price
                    FROM bookings b
    ...
    def calc_booking_value(self, b: dict) -> float:
        checkin = b.get('checkin')
        checkout = b.get('checkout')
        preco_diaria = float(b.get('daily_price'))
        ...
        # Regra: se entrar e sair no mesmo dia, cobra 1 diária
        if diarias <= 0:
            diarias = 1
