...
def fmt_booking(b: dict) -> str:
    ...
    # Chamamos a função de cálculo que criamos acima
    try:
            valor_total = controller.calc_booking_value(b)
            # Calcula as diárias para exibir (ex: 2x diária)
            dias = (b['checkout'] - b['checkin']).days
            diarias = max(1, dias) if b.get('checkout') else 0
            
            total_str = f"R$ {valor_total:.2f} ({diarias}x diária)"
    except Exception as e:
        print(f"Erro ao calcular: {e}")
        total_str = "Erro no cálculo"
    
    checkin_str = b['checkin'].strftime("%d/%m/%Y") if b.get('checkin') else "?"
    checkout_str = b['checkout'].strftime("%d/%m/%Y") if b.get('checkout') else "?"
    
    total_str = f"R$ {valor_total:.2f} ({diarias}x diária)"
    
    return (
        f"  ID: {b.get('id')} | "
        f"Cliente: {b.get('client_name')} | "
...
def ver_reservas_cliente(cliente: Client):
    # 1. Ajuste no print: use .get_name()
    print(f"\n   Reservas de {cliente.get('name')}") 
    
    reservas = [
        b for b in controller.get_all_bookings().values()
        # 2. Ajuste na comparação: use .get_cpf()
        if b.get('client_cpf') == cliente.get('client_cpf')
    ]
